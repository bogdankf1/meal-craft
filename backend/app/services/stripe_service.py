...
class StripeService:
    ...
    async def create_checkout_session(
        ...
        Args:
            ...
            price_id: Stripe price ID (Chef's Choice or Master Chef tier)
    ...
    async def handle_checkout_completed(
        ...
        if price_id == settings.STRIPE_PRICE_PLUS:
            tier_name = SubscriptionTier.PLUS
        elif price_id == settings.STRIPE_PRICE_PRO:
            tier_name = SubscriptionTier.PRO
        else:
    ...
    async def handle_subscription_updated(
        ...
        # Get user by stripe_subscription_id or stripe_customer_id
        result = await db.execute(
            select(User).where(
                (User.stripe_subscription_id == subscription_id) |
                (User.stripe_customer_id == customer_id)
