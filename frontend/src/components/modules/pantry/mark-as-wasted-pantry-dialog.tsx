...
import {
  ...
  type PantryWasteReason,
  PANTRY_WASTE_REASONS,
} from "@/lib/api/pantry-api";

interface MarkAsWastedPantryDialogProps {
  open: boolean;
...
export function MarkAsWastedPantryDialog({
  open,
  onOpenChange,
  items,
  onSuccess,
}: MarkAsWastedPantryDialogProps) {
  const t = useTranslations("pantry.waste");
  const tCommon = useTranslations("common");

  const [wasteReason, setWasteReason] = useState<PantryWasteReason>("expired");
  const [wasteNotes, setWasteNotes] = useState("");

  const [markAsWasted, { isLoading: isMarkingOne }] = useMarkPantryItemAsWastedMutation();
  const [bulkMarkAsWasted, { isLoading: isMarkingBulk }] = useBulkMarkPantryItemsAsWastedMutation();
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              value={wasteReason}
              onValueChange={(value) => setWasteReason(value as PantryWasteReason)}
            >
              <div className="grid grid-cols-1 gap-2">
                {PANTRY_WASTE_REASONS.map((reason) => (
                  <div
                    key={reason.value}
                    className="flex items-center space-x-2 p-2 rounded-md hover:bg-muted"
                  >
                    <RadioGroupItem value={reason.value} id={`pantry-${reason.value}`} />
                    <Label
                      htmlFor={`pantry-${reason.value}`}
